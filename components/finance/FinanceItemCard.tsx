...
import { FiArrowDown, FiArrowUp, FiCheckCircle, FiCircle, FiTrash2 } from "react-icons/fi";
import { deleteFinanceItem, toggleStatus } from "@/app/[locale]/tools/finance/(protected)/actions";
...
export default function FinanceItemCard({ item, locale }: Props) {
    ...
    const handleDelete = async () => {
        if (confirm("Tem certeza que deseja apagar?")) {
            await deleteFinanceItem(item.id, locale);
        }
    };

    const handleToggle = async () => {
        await toggleStatus(item.id, item.status, locale);
    };
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between gap-3 mb-3">
            ...
            <div className="text-right flex flex-col items-end gap-1">
                ...
                <div className="flex items-center gap-3 mt-1">
                    <button onClick={handleToggle} className={`${isPaid ? 'text-green-500' : 'text-gray-300'} hover:text-green-600 transition`}>
